...
pub trait StoreShard<'a> {
    ...
    fn new(parent: Self::Store, name: &str) -> Result<Self, Self::Error> where Self: Sized;
    ...
    fn insert(&self, item: Self::Item, metadata: Vec<Self::MetadataItem>) -> Result<Self::ObjectId, Self::Error>;
    ...
    fn find(&self, meta: Vec<Self::MetadataItem>) -> Result<Option<Vec<Self::ObjectId>>, Self::Error>;
}
...
impl<'a> StoreShard<'a> for DefaultStorageShard {
    ...
    fn new(parent: storage::DefaultStore, name: &str) -> Result<Self, String> where Self: Sized {
        let objects = parent
            .get_inner()
        ...
        Ok(Self {
            name: format!("{name}"),
            parent, objects, labels, metadata,
        })
    }
